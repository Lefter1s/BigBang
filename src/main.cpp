...
unsigned int nStakeMinAge = 24 * 60 * 60; // 1 day minimum stake
unsigned int nStakeMaxAge = -1;           //maximum stake is unlimited
...
int nCoinbaseMaturity = 140;//150 blocks to mature
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // BigBang: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // BigBang: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    if (pindexBest->nHeight < 1)
      {
        int64_t nSubsidy = 63000 * COIN; //0.9% Premine
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
	...
	{
    int64_t nSubsidy = 30 * nCoinAge * nRewardCoinYear / 365 / COIN;
	...
	{
	int64_t nSubsidy = 10 * nCoinAge * nRewardCoinYear / 365 / COIN;
...
// BigBang: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // BigBang: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // BigBang: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // BigBang: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // BigBang: track money supply and mint amount info
...
// BigBang: total coin age spent in transaction, in the unit of coin-days.
...
// BigBang: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // BigBang: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // BigBang: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // BigBang: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // BigBang: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // BigBang: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // BigBang: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // BigBang: getblocks may not obtain the ancestor block rejected
    ...
    // BigBang: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BigBang", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "BigBang x15 Coin with random rewards and superblocks!!";
        CTransaction txNew;
        txNew.nTime = 1407685738;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1407685738;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 4356819;
        if(fTestNet)
        {
            block.nNonce   = 153134;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("16f9b8a5f0fce7157838c1cceaefc13a6b6b6937c78683bc5f3b219c53c05e14"));
        block.print();
        ...
        // BigBang: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // BigBang: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // BigBang: tell downloading node about the latest block if it's
